"""Tests for the ``_inject_nextcloud_bridge`` build step in ``osprey.cli.build_cmd``.

Covers the three responsibilities of the nextcloud-bridge-injection step: copying
the bundled ``templates/services/nextcloud_bridge/`` compose template, writing
the ``services.nextcloud_bridge`` config + registering it in
``deployed_services`` (additively and idempotently), and — the part a
config-level assertion cannot reach — that the config block it writes is
*sufficient to render the template*. The compose template reads
``services.nextcloud_bridge.trigger`` with no ``| default``, so a missing key
would render an empty ``DISPATCH_TRIGGER`` and produce a bridge that either
crash-loops or fires a trigger nobody declared. The render tests therefore
assert on the rendered compose text, not merely on a zero exit code.
"""
...
from osprey.cli.build_cmd import _inject_nextcloud_bridge, build
...
def _render_bridge_compose(project_path: Path) -> str:
    ...
    template = env.get_template("services/nextcloud_bridge/docker-compose.yml.j2")
...
def _build_bridge_project(
    runner: CliRunner, tmp_path: Path, *, nextcloud_bridge: str = "nextcloud_bridge: {}"
) -> tuple[Path, object]:
...
def test_inject_nextcloud_bridge_copies_template_dir(tmp_path: Path) -> None:
    """The bundled compose template dir is copied into services/nextcloud_bridge."""
    project_path = tmp_path / "project"
    project_path.mkdir()
    _write_config(project_path)

    _inject_nextcloud_bridge(NextcloudBridgeProfileConfig(), project_path=project_path)

    dest = project_path / "services" / "nextcloud_bridge"
    assert (dest / "docker-compose.yml.j2").is_file()
    assert (dest / "Dockerfile").is_file()
    assert (dest / ".dockerignore").is_file()


def test_inject_nextcloud_bridge_writes_service_config(tmp_path: Path) -> None:
    """services.nextcloud_bridge is written with path + trigger, and
...
def test_inject_nextcloud_bridge_writes_custom_trigger(tmp_path: Path) -> None:
    ...
    _inject_nextcloud_bridge(
        NextcloudBridgeProfileConfig(trigger="other-trigger"), project_path=project_path
    )

    assert _read_config(project_path)["services"]["nextcloud_bridge"]["trigger"] == "other-trigger"


def test_inject_nextcloud_bridge_deployed_services_idempotent(tmp_path: Path) -> None:
    ...
    config = _read_config(project_path)
    deployed = [str(s) for s in config["deployed_services"]]
    assert deployed.count("nextcloud_bridge") == 1
    # And the second run left one well-formed service block, not a merged mess.
    assert config["services"]["nextcloud_bridge"]["path"] == "./services/nextcloud_bridge"


def test_inject_nextcloud_bridge_missing_config_yml_is_noop(tmp_path: Path) -> None:
...
def test_inject_nextcloud_bridge_ships_the_partials_its_template_imports(tmp_path: Path) -> None:
...
def test_inject_nextcloud_bridge_renders_without_dispatch_pair_deployed(tmp_path: Path) -> None:
    ...
    svc = doc["services"]["nextcloud-bridge"]
    ...
    # a handle_event raise leaves the persisted offset unadvanced, the room would
    # re-fetch the same batch forever. The `:?` guard is what makes that a
    ...
    assert env["DISPATCH_TRIGGER"] == "nextcloud-question"
...
def test_full_build_with_bridge_renders_service_dir(runner: CliRunner, tmp_path: Path) -> None:
    ...
    assert "nextcloud_bridge" in deployed
    # Step 10b2 runs after 10b, so the dispatch pair the template gates on is
...
def test_full_build_compose_renders_with_no_unrendered_jinja(
    ...
    it renders an empty value (or, for `services.nextcloud_bridge`, an undefined
    ...
    svc = yaml.safe_load(rendered)["services"]["nextcloud-bridge"]
    ...
    # Bot credentials stay BARE — deliberately, and for a different reason than the
    # URLs above. A bare `${VAR}` is not fail-closed at the compose layer (unset
    # renders an empty string, not an absent key); these fail closed one layer up,
    # in NextcloudBridgeConfig.require_startup, whose falsy check treats "" as
    # missing and aborts naming every missing var at once — a better operator
    # message than compose's per-variable `:?`. What must never appear here is a
    # `:-` default, which would boot the bridge against a guessable host or
    # password. Do not "harmonise" these with the `:?` form above without moving
    # the require_startup contract too.
    for var in (
        "NEXTCLOUD_BASE_URL",
    ...
    assert env["TZ"] == "UTC"
    assert svc["volumes"] == ["nextcloud_bridge_data:/data"]
...
def test_full_build_custom_trigger_reaches_rendered_compose(
    ...
    project_path, result = _build_bridge_project(
        runner, tmp_path, nextcloud_bridge="nextcloud_bridge:\n  trigger: other-trigger"
    )
    assert result.exit_code == 0, result.output

    svc = yaml.safe_load(_render_bridge_compose(project_path))["services"]["nextcloud-bridge"]
...
def test_full_build_without_bridge_scaffolds_no_bridge_service(
    ...
    """The block is opt-in: a profile without it deploys no bridge (step 10b2 skipped)."""
    project_path, result = _build_bridge_project(runner, tmp_path, nextcloud_bridge="")
    assert result.exit_code == 0, result.output

    config = yaml.safe_load((project_path / "config.yml").read_text(encoding="utf-8"))
    assert "nextcloud_bridge" not in config["deployed_services"]
    assert "nextcloud_bridge" not in config.get("services", {})
    assert not (project_path / "services" / "nextcloud_bridge").exists()
...
def test_full_build_unknown_trigger_aborts(
    ...
    errors = _build_errors(caplog)
    assert "Build profile validation failed" in errors
    assert "nextcloud_bridge.trigger 'ghost' is not declared in" in errors
    # The declared alternatives are named so the operator can fix it in one pass.
    assert "nextcloud-question" in errors
    assert not (project_path / "services" / "nextcloud_bridge").exists()
